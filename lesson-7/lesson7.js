...
var danger_timer; // Таймер для еды
...
function createDanger() {
    var danderCreated = false;

    while (!danderCreated) { //пока препятствие не создали
        // рандом
        var danger_x = Math.floor(Math.random() * FIELD_SIZE_X);
        var danger_y = Math.floor(Math.random() * FIELD_SIZE_Y);

        var danger_cell = document.querySelector('.cell-' + danger_y + '-' + danger_x);
        ...
        if (!danger_cell_classes.includes('snake-unit')) {
            ...
            danger_cell.classList.add("danger-unit");
            danderCreated = true;
        }
    }
}
